...
export default function LoginPage() {
  ...
  if (user) { // If user becomes available while component is mounted but before useEffect redirects
    return (
      <div className="flex justify-center items-center min-h-[calc(100vh-8rem)]">
        ...
        <CardContent className="space-y-6">
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={isLoggingIn || authLoading}>
              {isLoggingIn ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <LogInIcon className="mr-2 h-4 w-4" />}
              Sign In with Email
            </Button>
          </form>
          ...
          <Button variant="outline" className="w-full" onClick={handleGoogleLogin} disabled={isLoggingIn || authLoading}>
             {isLoggingIn && !email ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 
